...
/**
 * Registry process that allows peers in the system to register as peers and ask
 * for
...
public class Registry {
	/**
	 * Number of threads allowed in the system. Each thread manages communication
	 * with a peer
	 */
	public static final int THREAD_POOL_SIZE = 10;
	/**
	 * Maximum number of peer addresses that will be send when communicating with a
	 * peer
	 */
	public static final int MAX_NUM_OF_PEERS = 4;
	/**
	 * If no port number is provided when running the registry, this port number
	 * will be used.
	 */
	public static final int DEFAULT_PORT_NUMBER = 55921;

	/**
	 * Length of time to run the system and accept new peer connections before
	 * shutting
	 * the system down and wait for reports from peers in the system. Note that the
	 * time
	...
	/**
	 * Length of time to wait for connection from peers after shut down message was
	 * multicast.
	...
	/**
	 * Length of time we'll wait for ack of stop message before resending the
	 * message.
	 */
	 ...
	 * thread pool.
	 * 
	 * @throws IOException if there are problems starting this registry server or if
	 *                     there
	 *                     are problems communication along a connection with a
	 *                     peer.
	 */
	public void start() throws IOException {
		ExecutorService executor = Executors.newFixedThreadPool(THREAD_POOL_SIZE);

		// This thread will look for command line input: right now the only command is
		// 'done'
	...
	/**
	 * Creates a thread that monitors for input from the command line. Right now, we
	 * are only
	 * interested in the command 'done' that indicates we should start the process
	 * of shutting the
	 * entire system down.
	 * <p>
	 * It will only let all peers in the system know to shut down. The actually
	 * shutting down of this
	...
	/**
	 * Keeps track of timers to shut system down and then restart after sufficient
	 * time has
	 * passed to get all reports from peers. The amount of time the system will run
	 * and that the
	 * system will wait for reports is in the constants MINUTES_TO_RUN_SYSTEM and
	 * MINUTES_TO_WAIT_FOR_REPORT.
	 */
	...
	/**
	 * Reveive messages coming in along this UDP socket. The only messages we're
	 * interested
	 * in are ack message which are expected to be in the format:
	 * ack<team name>
	 * <p>
	 * All other messages are ignored. This method will continue reading until the
	 * socket
	 * is closed.
	 * 
	...
	/**
	 * Flag that we are in the shutdown phase. We'll send a UDP message to each peer
	 * to let them know
	 * to end their work and re-connect with the registry to submit a final report.
	 */
	private void done() {
		done = true;
		// Let all processes we know about that we're done and that they should shut
		// down.
		// TODO: create multiple threads so we can communicate with multiple peers
		// simultaneously
		try {
					...
					if (!p.acked) {
						try {
							...
							DatagramPacket packet = new DatagramPacket(msg, msg.length,
									InetAddress.getByName(p.address), p.getPort());
	...
	/**
	 * Place the specified in the list of know peers. If this peer already exists,
	 * the
	 * existing peer will be replaced ensuring that the list of peers send to this
	 * new/updated
	 * peer is maintained.
	 * 
	...
	/**
	 * Starts the registry server. If a port number is provided as a runtime
	 * argument,
