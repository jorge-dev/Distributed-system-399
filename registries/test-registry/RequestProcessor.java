...
/**
 * Process a connection request from a peer in our system. A request is
 * processed by
 * requesting the team name, the source code produced by the team and by sending
 * a
 * (partial) list of peers in the system. At the end of the communication, we'll
 * ask
 ...
 * 'get report'<br>
 * 'close'<br>
 * It is the best version to test your code since the order of operations is
 * predictable. A peer
 * should be able to handle these requests in any order however: the version
 * we'll use for testing
 * can send requests in any order and can duplicate requests. The only guarantee
 * is that the 'close'
 * request will be the last request.
 * <p>
 * Information of all peer processes that connect with this registry will be
 * stored. The
 * registry should be restarted to clear the peer list on a regular bases. In
 * future the peer list
 * will be maintained based on aliveness of the peer.
 * <p>
 * The class logs all communication for verification purposes, including any
 * errors in
 * communication.
 * 
...
class RequestProcessor implements Runnable {
	...
	/**
	 * The registry that received the connection request: it handles the list of
	 * peers
	 */
	private Registry source = null;

	/**
	 * Setup up the logger to store logging messages in the file
	 * 'registryLogger.log' in the
	 * current directory.
	 */
	static {
		try {
	 ...
	 * 
	 * @param aPeer   the socket connection with the peer whose request we need to
	 *                process.
	 * @param aSource the registry that received the connection request from the
	 *                peer.
	 */
	...
	/**
	 * Handles the communication with the peer that we are connected with. It will
	 * first
	 * ask the name of the team that created this peer, store the information for
	 * this peer
	 * process, send a list of know peers, request a report from the peer then close
	 * the
	...
	/**
	 * We are done with this connection. Warn other end that we are going to close
	 * the connection.
	 * <p>
	 ...
	 * 
	 * @throws IOException if the connection with the peer is broken unexpectedly or
	 *                     we are unable to
	...
	/*
	 * Sends a list of peers in the system along the socket. If the current peer
	 * object
	 * that we created for this channel already contains such a list of peers, we'll
	 * send this
	 * list. If it doesn't contain a list yet, we'll request the source Registry
	 * object
	 ...
	 * 
	 * @throws IOException if the channel is unexpected closed or we are unable to
	 * send data
	 * along the channel.
	 */
	protected void sendPeers() throws IOException {
		...
		// let peer know that we are about to send peers and they should get ready to
		// receive
		...
		if (peer == null || peer.peersSent == null || peer.peersSent.length < Registry.MAX_NUM_OF_PEERS) {
			// create a new list to send along the channel since none was set up for this
			// peer yet.
	...
	/*
	 * Get the name of the team that created the code for this peer. Communication
	 * protocol:
	 * 
	 * send: 'get team name\n'
	...
	/*
	 * Request the source code that the peer we are connected with is running.
	 * Communication
	 * protocol:
	 * send 'get code\n'
	 ...
	 * receive '...'
	 * 
	 * @return The date the code was received, the language the code was written in
	 * and the
	...
	/*
	 * Store the source code of the peer that we are currently connected with in a
	 * file.
	 ...
	 * language used. The name of the file created contains the team name followed
	 * by
	 * 'SourceCode.txt'.
	 */
	...
	/**
	 * Create a peer object for the current connection. Request the information from
	 * the peer
	 * along the channel with the peer and store this information. Information
	 * requested:
	 * team name, and the code that is used to run the peer.
	 * 
	 * @return the peer object created based on the info received from the peer
	 *         along the channel.
	 * @throws IOException if the connection with the peer is unexpectedly closed or
	 *                     if
	...
	protected Peer addPeer() throws IOException {
		...
		Peer peerToAdd2 = new Peer();
		peerToAdd2.address = "1.1.1.1";
		peerToAdd2.port = 2222;
		peerToAdd2.teamName = "random";
		source.addPeer(peerToAdd2);
	...
	/**
	 * Ask the peer for a report of it's actions. This should include the peers it
	 * has stored in
	 * it's list of peers and a report of communication. For this iteration, the
	 * only
	 * communication is with this Registry object.
	 * <p>
	 * The report for this team will be stored in the directory 'reports' in the
	 * file
	 * <team name>Report.txt
	 * 
	 * @throws IOException if there is any problems communicating with the peer for
	 *                     this report request.
	 */
	...
	/**
	 * Read the report send by the peer. The format of the report is expected to be
	 * as
	 ...
	 *
	 * All data is expected to be textbase. (So numbers should be send as a string,
	 * not a number.)
	 * 
	 * This information will be returned as a string in the following format:
	 * <current date/time><newline>
	 * <num of peers> in list: <ip_peer1>:<port_peer1> <ip_peer2>:<port_peer2> ...
	 * <ip_peer_n>:<port_peer_n><newline>
	 * <num of sources> sources:<newline>
	 * Source: <ip><port><newline>
	 * Date: <date><newline>
	 * Num of peers received: <numOfPeers><newline>
	 * <ip_peer1>:<port_peer1> <ip_peer2>:<port_peer2> ...
	 * <ip_peer_k>:<port_peer_k><newline>
	 * 
	 ...
	 * @throws IOException if there were any problems while receive input from the
	 *                     peer
	 */
	...
	protected void log(Level level, String message) {
		String logMessage = ((peerSocket != null) ? peerSocket.getRemoteSocketAddress().toString() + " " : "") +
				message;
		LOGGER.log(level, logMessage);
	}

	/**
	 * Format logging message that resulted from exception to include peer
	 * information
	 * 
	 * @param level   severity level of log message
	 * @param message data for log message
